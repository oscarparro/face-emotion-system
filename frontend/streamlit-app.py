...
with tab1:
    ...
    with st.form("mode_form", clear_on_submit=False):
        ...
        with cols[0]:
            selected_mode = st.selectbox("Modo de funcionamiento",
...
#########################
with tab2:
    # Inicializa el flag (solo la primera vez)
    if "register_mode" not in st.session_state:
        st.session_state.register_mode = False

    # Botón que alterna entre mostrar/ocultar
    if st.button("Activar Registro", key="toggle_register"):
        st.session_state.register_mode = not st.session_state.register_mode

    # Si el flag está activo, mostramos todo el UI de captura
